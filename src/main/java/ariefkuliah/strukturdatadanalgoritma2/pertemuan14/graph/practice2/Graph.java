package ariefkuliah.strukturdatadanalgoritma2.pertemuan14.graph.practice2;
...
public class Graph {
     ...
     * Vertex.
     */
    static String[] kota = {"A", "B", "C", "D", "E", "F", "G", "H"};
        ...
        for (int kolom = 0; kolom < edges.length; kolom++) {
            for (int baris = 0; baris < edges.length; baris++) {
                if (edges[kolom][baris]) {
                    hitung++;
                    if (Arrays.equals(edges, unDirectGraph)) {
                        System.out.println("(" + kota[kolom] + "," + kota[baris] + ")" + " Baris: " + baris + " Kolom: " + kolom + " Perulangan ke-" + hitung);
                    }
                    if (Arrays.equals(edges, directGraph)) {
                        System.out.println("<" + kota[kolom] + "," + kota[baris] + ">" + " Baris: " + baris + " Kolom: " + kolom + " Perulangan ke-" + hitung);
                    }
